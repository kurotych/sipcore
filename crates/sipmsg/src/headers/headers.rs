...
pub struct Headers<'a> {
    headers: BTreeMap<Ascii<&'a str>, VecDeque<SipHeader<'a>>>,
}

impl<'a> Headers<'a> {
    pub fn get(&self, key: &'a str) -> Option<&VecDeque<SipHeader<'a>>> {
        self.headers.get(&Ascii::new(key))
    }

    /// get single value
    /// Returns some value if header by key should be present only one time
    pub fn get_s(&self, key: &'a str) -> Option<&SipHeader<'a>> {
...
impl<'a> NomParser<'a> for Headers<'a> {
    ...
    fn parse(input: &'a [u8]) -> nom::IResult<&[u8], Self::ParseResult, SipParseError> {
        let mut headers = BTreeMap::<Ascii<&'a str>, VecDeque<SipHeader<'a>>>::new();
        let mut inp2 = input;
        loop {
